...
plt.title('DATA011 FFT Spectrum Comparison')
# Zoom near region of interest
plt.xlim(0.1,21) # up to Nyquist (fs/2 = 200)
plt.ylim(-170,-30)
#Find and mark peak
...
plt.legend()
plt.tight_layout()
plt.savefig('C:/Users/giese/OneDrive/Documents/Work/Research/Shaker Data Collection/DATA011_FFT_Spectrum_Comparison.png')

plt.figure(2)
plt.plot(tt1, ac1 / 9.8, label = 'Color SD Card')
plt.plot(tt2, ac2 / 9.8, label = 'Black SD Card')
plt.xlabel('Time (s)')
plt.ylabel('Acceleration (g)')
plt.xlim(0,300)
plt.title('DATA011 Acceleration Comparison')
plt.legend()
plt.tight_layout() # makes sure plot is neat and sizings work together
plt.savefig('C:/Users/giese/OneDrive/Documents/Work/Research/Shaker Data Collection/DATA011_Acceleration_Comparison.png', dpi=300)
...
plt.title('DATA011 Frequency Response Function (Magnitude)')
plt.xscale('log')
plt.xlim(0.1,21) # Nyquist Freq
plt.tight_layout()
plt.savefig('C:/Users/giese/OneDrive/Documents/Work/Research/Shaker Data Collection/DATA011_FRF_Magnitude.png', dpi=300)
...
# plt.ylim(-200, 200)
# plt.tight_layout()
# plt.savefig('C:/Users/giese/OneDrive/Documents/Work/Research/Shaker Data Collection/DATA011_FRF_Phase.png', dpi=300)


window_size = 10
A1_smooth = np.convolve(A1_dB, np.ones(window_size)/window_size, mode='same')
A2_smooth = np.convolve(A2_dB, np.ones(window_size)/window_size, mode='same')

plt.figure(5)
plt.plot(f1, A1_smooth, label = 'Color SD Card')
plt.plot(f2, A2_smooth, label = 'Black SD Card')
plt.grid(True)
plt.ylabel('Amplitude (dB)')
plt.xlabel('Frequency (Hz)')
plt.title('DATA011 FFT Smoothed Spectrum Comparison')
# Zoom near region of interest
plt.xlim(0.1,21) # up to Nyquist (fs/2 = 200)
plt.ylim(-170,-30)
#Find and mark peak
peak_idx1 = np.argmax(A1_smooth)
peak_idx2 = np.argmax(A2_smooth)
plt.plot(f1[peak_idx1], A1_smooth[peak_idx1], 'ro')
plt.plot(f2[peak_idx2], A2_smooth[peak_idx2], 'go')
plt.text(f1[peak_idx1] + 2, A1_smooth[peak_idx1], f'{f1[peak_idx1]:.1f} Hz', color = 'red')
plt.text(f2[peak_idx2] + 2, A2_smooth[peak_idx2], f'{f2[peak_idx2]:1} Hz', color = 'green')
plt.legend()
plt.tight_layout()
plt.savefig('C:/Users/giese/OneDrive/Documents/Work/Research/Shaker Data Collection/DATA011_FFT_Smoothed_Spectrum_Comparison.png')