...
# tt= time, ac = acceleration
    ...
    # FFT computation
    N = len(ac_filtered)                         # number of samples
    Y = np.fft.fft(ac_filtered)         # FFT
    ...
    return tt, ac, ac_filtered, f, A, A_dB


#%% Process both files
tt1, ac1, ac1_filtered, f1, A1, A1_dB = process_data(file1)
...
plt.plot(tt1, ac1_filtered, label='High Pass Filtered')
plt.xlabel('Time (s)')
plt.ylabel('Acceleration (g)')
plt.legend()
plt.grid(True)
