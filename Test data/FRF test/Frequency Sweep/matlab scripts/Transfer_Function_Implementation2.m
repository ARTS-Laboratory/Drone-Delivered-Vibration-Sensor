...
%% Load VI Data 
D = dlmread('C:\Users\joud\OneDrive - University of South Carolina\Mech research\test\FRF test\Frequency Sweep\Test14 model validation\FRF Test Fs VI 1650Hz 2.lvm',',',23);
...
D2 = dlmread('C:\Users\joud\OneDrive - University of South Carolina\Mech research\test\FRF test\Frequency Sweep\Test14 model validation\FRF Test Fs pkg 1650Hz 2.lvm',',',0);
...
%% Interpolate VI time data and package to Nyquist Bandwidth (40Hz)
tt40=0: 1/48 :43.83416367; %time column for Fs=40Hz
VIAcc = interp1(tt,-dd,tt40); %MULTIPLIED BY A NEGATIVE TO MATCH THE ORIENTATION
PkgAcc = interp1(tt2,PkgAccCal,tt40);

%time domain interpolated:
figure(3);
plot(tt40,VIAcc,tt40,PkgAcc);
grid on
title('Data Interpolated to Fs=40Hz');
xlabel("time (s)");
ylabel("acceleration (m/s^2) ");
...
AccOT = movmean(AccOTX,30);
AccIN = movmean(AccINX,30);

%Plot the averaged FFTs
plot(f,AccOT,f2,AccIN);
title('FFT at Fs=40Hz');
xlabel('frequency (Hz)');
ylabel('|G(f)|');
%ylim([0,.05]);
%xlim([0.01,20]);
